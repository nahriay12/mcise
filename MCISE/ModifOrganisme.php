<?php 
    ...
    if(isset($_GET['edit'])){
		$id=$_GET['edit'];
		$Org=$fonction->Show('organisme',$id,'id_O');
	}
	$chaine=array(explode('@',$Org->Sec_Org,2));
	unset($chaine[0][0]);
	$sec=explode('@',$chaine[0][1]);
...
<body class="onepage">
 <div id="load"></div>
	...
	<div class="title">
		<h1>Modifier Organisme</h1>
	</div>
	<form method="post" enctype="multipart/form-data" action="Organisme.php?page=Modifier">
		<div class="input-group">
			<table>
			<tr>
				<td><label>Nom de l'organisme </label></td>
				<td><input type="text" name="nom" value="<?=$Org->Nom?>" required></td>
				<td><input type="hidden" name="id" value="<?=$Org->id_O?>"></td>
			</tr>
			<tr>
				<td><label>Description</label></td>
				<td><textarea name="desc" cols="40" rows="4" placeholder="Description de cette organisation....."><?=$Org->Description?></textarea></td>
			</tr>
			<tr>
				<td><label>Email </label></td>
				<td><input type="text" name="email" value="<?=$Org->Email?>" required> </td>
			</tr>
			<tr>
				<td><label>Phone </label></td>
				<td><input type="text" name="phone" value="<?=$Org->Tel?>" required> </td>
			</tr>
			<tr>
				<td><label>Adresse </label></td>
				<td><input type="text" name="adresse" value="<?=$Org->Adresse?>" required></td>
			</tr>
			<tr>
				<td><label>Site web </label></td>
				<td><input type="text" name="site" value="<?=$Org->Site_Web?>" required></td>
			</tr>
			<tr>
				<td><label>Image</label></td>
				<td><input type="file" name="img"> 
				<img src="images/portfolio/<?=$Org->Image?>"></td>
			</tr>
			<tr>
				...
				<td>
					<ul>
						<?php foreach($Secteurs_afiiche as $s):?>
							<input 	type="checkbox" 
									name="Sec[]" 
									value="<?php echo $s->Id_S;?>" 
									<?php foreach($sec as $s2){if($s->Id_S==$s2){echo "checked";}}?>>
									<?php echo $s->Nom;?><br>
						<?php endforeach?>
					</ul>
				</td>
			...
			<tr>
				<td>
					<label style='color:red;'>L'image est obligatoire !!!!!</label>
				</td>
				<td>
